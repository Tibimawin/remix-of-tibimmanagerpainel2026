...
export const FloatingSidebar: React.FC<FloatingSidebarProps> = ({
  ...
  const checkScrollability = useCallback(() => {
    ...
    if (container) {
      ...
      if (viewport) {
        const hasMoreContent = viewport.scrollHeight > viewport.clientHeight;
        const isNotAtBottom = viewport.scrollTop + viewport.clientHeight < viewport.scrollHeight - 10;
  ...
  }, [checkScrollability, isVisible]);

  useEffect(() => {
    // Re-check when menus expand/collapse
    const timer = setTimeout(checkScrollability, 100);
    return () => clearTimeout(timer);
  }, [openMenus, checkScrollability]);
          ...
          <div className="flex-1 min-h-0 relative">
            ...
            <div
              className={cn(
                ...
                "bg-gradient-to-t from-card/95 via-card/60 to-transparent",
                "flex items-end justify-center pb-1",
                canScrollDown ? "opacity-100" : "opacity-0"
              )}
            >
              <div className="flex items-center gap-1 text-primary/70 animate-bounce">
                <ChevronsDown className="w-4 h-4" />
                <span className="text-xs font-medium">Mais itens</span>
              </div>
            </div>
          </div>
